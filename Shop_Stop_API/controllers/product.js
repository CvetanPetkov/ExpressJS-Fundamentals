...
module.exports.editGet = (req, res) => {
  ...
  Product.findById(id).then((product) => {
    ...
    if (product.buyer) {
      res.redirect(`/?error=${encodeURIComponent('Request Error: Product is locked and can not be modified!')}`)
      return
    }
...
module.exports.deleteGet = (req, res) => {
  ...
  Product
    ...
    .then((product) => {
      ...
      if (product.buyer) {
        res.redirect(`/?error=${encodeURIComponent('Request Error: Product is locked and can not be deleted!')}`)
        return
      }
...
module.exports.deletePost = (req, res) => {
  ...
  Product
    ...
    .then((currProduct) => {
      ...
      if (currProduct.buyer) {
        res.redirect(`/?error=${encodeURIComponent('Request Error: Product is locked and can not be deleted!')}`)
        return
      }

      if (currProduct.creator.equals(req.user._id) || req.user.roles.indexOf('Admin') >= 0) {
        Category
